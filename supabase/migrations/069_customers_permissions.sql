-- FleetControl Sprint 24 — Customers RBAC permissions

insert into public.permissions (code, resource, action, description)
values
  ('customers:read',   'customers', 'read',   'View customers and contracts'),
  ('customers:create', 'customers', 'create', 'Create customers and contracts'),
  ('customers:update', 'customers', 'update', 'Update customers and contracts'),
  ('customers:delete', 'customers', 'delete', 'Delete customers and contracts')
...
where r.deleted_at is null
  and p.code in ('customers:read', 'customers:create', 'customers:update', 'customers:delete')
  and (
    r.name in ('Super Admin', 'Admin')
    or (r.name = 'Manager' and p.code in ('customers:read', 'customers:create', 'customers:update'))
    or (r.name = 'Operator' and p.code = 'customers:read')
  )
on conflict do nothing;

-- Update canonical role seeding to include customer permissions
...
comment on function public.seed_default_roles_for_company(uuid, uuid) is
  'Canonical role seeding — creates system roles and maps permissions (Sprint 24)';
