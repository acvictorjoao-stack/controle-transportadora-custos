...
import {loadOperationalDreDriverTripsAction} from '@/features/dre/actions';
...
export interface OperationalDreDriverCostsProps {
  data: OperationalDreByDriverData;
...
/**
 * Drill-down Motorista → Viagens → breakdown financeiro.
 */
function OperationalDreDriverCosts({
  data,
  filters,
  basePath = ROUTES.dashboardRentabilidadeMotoristas,
  comparisons,
  onActiveTrailChange,
}: OperationalDreDriverCostsProps) {
  ...
  const groupColumns = React.useMemo<
    AnalyticalExpandableColumn<OperationalDreDriverGroup>[]
  >(
    () => [
      {
        id: 'driver',
        ...
        cell: (row) => {
          ...
          return (
            <div className="space-y-1">
              <span className="font-medium">{row.driver.label}</span>
              {delta ? (
  ...
  const loadDetails = React.useCallback(
    async (group: OperationalDreDriverGroup) => {
      const result = await loadOperationalDreDriverTripsAction({
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Motorista → Viagens → Abastecimentos, Manutenções, Pneus e Financeiro.
          </p>
        </div>
      ...
      <AnalyticalExpandableTable
        ...
        expansionStorageKey="dre-driver-expanded"
        detailExpansionStorageKey="dre-driver-trip-financial-expanded"
        dataRevision={filtersKey}
        emptyTitle="Sem custos por motorista"
        emptyDescription="Viagens concluídas no período aparecerão agrupadas por motorista."
        detailEmptyTitle="Nenhuma viagem deste motorista para o filtro atual."
...
export {OperationalDreDriverCosts};
