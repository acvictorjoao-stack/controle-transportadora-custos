...
export async function listVehicleFuelRecordsForConsumption(
  ...
  const rows = await fetchAllPagedRows<FuelRow>(
    async (from, to) =>
      supabase
        .from('fuel_records')
        .select(
          'id, odometer_km, quantity_liters, total_amount, price_per_liter, fueled_at',
          {count: 'exact'},
        )
...
export async function listVehicleCompletedTripsForConsumption(
  ...
  const rows = await fetchAllPagedRows<TripRow>(
    async (from, to) =>
      supabase
        .from('trips')
        .select(
          'id, initial_odometer_km, final_odometer_km, route_id, customer_id, driver_id',
          {count: 'exact'},
        )
        .eq('company_id', companyId)
        .eq('vehicle_id', vehicleId)
